...
//    }
    ...
    fun testEmptyText() {
        ...
        assertEquals("Текст должен остаться прежним", initialText, result)
    }

    @Test
    fun testChangeTextAndLaunchNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        ...
        val newActivityPackage = "ru.netology.testing.uiautomator"
        device.wait(Until.hasObject(By.pkg(newActivityPackage).depth(0)), TIMEOUT)

        val resultText = device.findObject(By.res(newActivityPackage, "text")).text
        assertEquals("Текст в новой Activity должен совпадать с введённым текстом", resultText, textToSet)
    }
}
